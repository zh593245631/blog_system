...
typedef std::function<std::string(uint64_t offset, uint64_t length)> ContentProducer;
...
struct Response {
  ...
  void set_content_producer(uint64_t length, ContentProducer producer);
...
inline int write_content(Stream &strm, ContentProducer content_producer, uint64_t offset, uint64_t length) {
  uint64_t begin_offset = offset;
  uint64_t end_offset = offset + length;
  while (offset < end_offset) {
    auto chunk = content_producer(offset, end_offset - offset);
    offset += chunk.size();

    auto written_length = strm.write(chunk.c_str(), chunk.size());
    if (written_length < 0) { return written_length; }
  }
  return offset - begin_offset;
}

inline int write_content_chunked(Stream &strm, ContentProducer content_producer) {
  uint64_t offset = 0;
  auto data_available = true;
  auto total_written_length = 0;
  while (data_available) {
    auto chunk = content_producer(offset, 0);
    offset += chunk.size();
    ...
    if (std::regex_match(s, m, re)) {
      ...
      detail::split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
        ...
        if (std::regex_match(b, e, m, re)) {
          uint64_t first = std::stoll(m.str(1));
          if (!ranges.empty() && ranges.back().second >= first) {
            throw std::runtime_error("invalid range error");
          }
          uint64_t last = std::numeric_limits<uint64_t>::max();
          if (!m.str(2).empty()) {
            last = std::stoll(m.str(2));
          }
          if (first >= last) {
            throw std::runtime_error("invalid range error");
          }
...
inline std::pair<uint64_t, uint64_t> get_range_offset_and_length(const Request& req, const Response& res, size_t index) {
  ...
  if (r.second ==  std::numeric_limits<uint64_t>::max()) {
...
inline void Response::set_content_producer(uint64_t length, ContentProducer producer) {
  assert(length > 0);
  content_length = length;
  content_producer = producer;
}

inline void Response::set_chunked_content_producer(std::function<std::string(uint64_t offset)> producer) {
  content_length = 0;
  content_producer = [producer](uint64_t offset, uint64_t /*length*/) {
...
inline bool Server::write_response(Stream &strm, bool last_connection,
  ...
  if (res.body.empty()) {
    if (res.content_length > 0) {
      if (req.ranges.empty()) {
        res.set_header("Content-Length", std::to_string(res.content_length).c_str());
      } else if (req.ranges.size() == 1) {
        auto offsets = detail::get_range_offset_and_length(req, res, 0);
        auto length = offsets.second;
        res.set_header("Content-Length", std::to_string(length).c_str());
      } else {
  ...
  if (req.method != "HEAD") {
    if (!res.body.empty()) {
      ...
      if (res.content_length) {
        if (req.ranges.empty()) {
          if (detail::write_content(strm, res.content_producer, 0, res.content_length) < 0) { return false; }
        } else if (req.ranges.size() == 1) {
          auto offsets = detail::get_range_offset_and_length(req, res, 0);
          auto offset = offsets.first;
          auto length = offsets.second;
          if (detail::write_content(strm, res.content_producer, offset, length) < 0) { return false; }
        } else {
          // TODO: Support multiple ranges
          if (detail::write_content(strm, res.content_producer, 0, res.content_length) < 0) { return false; }
        }
      } else {
        if (detail::write_content_chunked(strm, res.content_producer) < 0) { return false; }
      }
    }
  }
